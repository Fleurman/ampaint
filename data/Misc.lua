...
function am.square(x,y,w,h,thickness,color)
  ...
  return {vec2(x,y), vec2(x,y+height), vec2(x+width,y+height), vec2(x+width,y)} end
  ...
  function wrapped:set_x(v) inner"translate".x = v end
  function wrapped:get_y() return y end
  function wrapped:set_y(v) inner"translate".y = v end
  function wrapped:get_scale() return height end
  function wrapped:set_scale(v) height,width = v,v
    for i,c in inner"group":child_pairs() do
      c.point1 = points()[i] local id = id<5 and i+1 or 4 c.point2 = points()[id]
  end end
